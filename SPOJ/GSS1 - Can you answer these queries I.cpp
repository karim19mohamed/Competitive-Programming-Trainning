...
ll arr[N];
struct node {
	ll best, pre, post, sum;

	node(ll val) {
		best = pre = post = sum = val;
	}
	node() {
		best = pre = post = INT_MIN;
		sum = 0;
	}
...
node operator+(const node &lf, const node &rt) {
	...
	ret.post = max(rt.post, rt.sum + lf.post);
	ret.sum = lf.sum + rt.sum;
	ret.best = max( { lf.best, rt.best, lf.post + rt.pre });
	return ret;
}
